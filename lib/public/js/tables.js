...
function showAllSteps(){
	var sol = document.getElementById("path-solution");
	...
	solution.forEach(function(node){
		setColorToNode(node, "open");
		if(solution.indexOf(node) < solution.length - 1){
			setColorToArrow(node, solution[solution.indexOf(node) + 1], "arrow");
		}
	});
...
function showStepByStep(){
	...
	switch(cases){
		case 0:
			...
			steps[stepByStep - 1].open.forEach(function(node){
				setColorToNode(node.name, "open");
			});

			setColorToNode(steps[stepByStep - 1].open[0].name, "selected-node");
		...
		case 1:
			...
			setColorToSelectedArrow(steps[stepByStep - 1].open[0], steps[stepByStep - 1].successors, "selected-arrow");
		...
		case 2:
			...
			setColorToSelectedArrow(steps[stepByStep - 1].open[0], steps[stepByStep - 1].successors, "");
